...
export default function ProjectCard (props) {
    ...
    function LangTab ({ size, color, lang }) {
        return (
            <div className="h-full" style={{ width: `${size}%`, backgroundColor: color }}>
                <p>{lang}</p>
            </div>
        )
    };
    ...
    return (
        ...
        transition ease-in-out duration-500 shadow-md shadow-neutral-400 dark:shadow-black" key={props.key}>
            ...
            <div className="flex flex-row items-end justify-center h-8 w-full bg-slate-500">
                {langTabs}
            </div>
        </div>
    )
